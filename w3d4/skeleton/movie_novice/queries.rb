...
def below_average_years
  ...
  Movie.select('yr, COUNT(title) bad_movies').where("score < ?", 5).group(:yr).order("yr DESC")
...
def pulp_fiction_actors
  ...
  Actor.select(:id, :name).joins(:castings).joins(:movies).where('title = ?', 'Pulp Fiction').distinct
end
...
def uma_movies
  ...
  # hint: use 'select', 'joins', 'where', and 'order'
    Actor.select('movies.id, movies.title, movies.yr').joins(:castings).joins(:movies).where('actors.name = ?', 'Uma Thurman').order('yr ASC').distinct
